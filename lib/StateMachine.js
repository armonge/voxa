...
class StateMachine {
  ...
  transition(alexaEvent, reply) {
    ...
    function runTransition() {
      return Promise.mapSeries(this.onBeforeStateChangedCallbacks, fn => fn(alexaEvent, reply, this.currentState))
        ...
        .then((transition) => {
          if (!transition) {
            ...
            return Promise.mapSeries(this.onUnhandledStateCallbacks, fn => fn(alexaEvent, this.currentState.name))
              .then(_.last);
          }
        ...
        .then((transition) => {
          ...
          return Promise.mapSeries(this.onAfterStateChangeCallbacks, fn => fn(alexaEvent, reply, transition))
            .then(() => transition);
        })
        .then((transition) => {
          let to;
          if (!transition) throw new errors.UnhandledState(transition, this.currentState.name);
          if (transition.to) {
            if (!this.states[transition.to]) {
              throw new errors.UnknownState(transition.to);
            }
            to = transition.to = this.states[transition.to];
